...
Como etapa inicial del procedimiento se realizó la identificación de los puntos anatómicos adecuados para la colocación de los electrodos seguido a ello con la limpieza de aquella zona. En este caso, se trabajó con una configuración bipolar ubicada en la región frontal, específicamente en las posiciones Fp1 y Fp2, correspondientes al sistema internacional 10–20 de colocación de electrodos para la medicion de EEG [2].
...
## Discusion:
Para analizar mejor la potencia de las diferentes bandas cerebrales se utilizó el método de Welch, ya que ofrece una estimación más estable que la FFT al reducir las fluctuaciones producidas por el ruido. Mientras que la FFT original presenta fluctuaciones significativas debido al ruido  y la varianza en segmentos cortos de señales de electroencefalografía (EEG), el método de Welch mitiga este problema al dividir la señal temporal en ventanas solapadas. Al promediar los espectrogramas parciales de estas ventanas, se reduce drásticamente la varianza, proporcionando una estimación espectral mucho más estable y robusta para la cuantificación de la potencia en cada ritmo cerebral ($\delta, \theta, \alpha, \beta, \gamma$).[3]
### Efecto del Filtrado (1-40 Hz):
Al comparar las gráficas, se observa que el filtrado pasabanda fue importante para mejorar la calidad de la señal. En la señal original, la banda Delta presentaba una potencia elevada; sin embargo, después de aplicar el filtro de 1-40 Hz, esta disminuyó notablemente. Esto indica que parte de la energía registrada en Delta probablemente estaba asociada a artefactos de baja frecuencia, como movimientos oculares, pequeñas variaciones de la línea base o micromovimientos del sujeto, y no únicamente a actividad cerebral. Además, el filtro también ayudó a reducir componentes por encima de 40 Hz, disminuyendo posibles ruidos de alta frecuencia e interferencias externas.[4]
### Análisis del Estado de Reposo:
Los resultados obtenidos en el sujeto evaluado coinciden con lo esperado para un estado de reposo. En el espectro filtrado, aunque la banda Delta mantiene una magnitud relativamente alta (algo que puede ocurrir cuando todavía existen pequeños movimientos o variaciones de fondo durante el registro), la banda Alfa se mantiene como la actividad predominante frente a las bandas de mayor frecuencia. Este predominio de Alfa suele asociarse a estados de relajación mental, especialmente cuando el sujeto se encuentra con baja carga cognitiva o en una condición tranquila.
Por otro lado, las bandas relacionadas con estados de mayor atención y procesamiento de información, como Beta y Gamma, presentan valores bajos. La banda Gamma prácticamente desaparece después del filtrado, mientras que Beta mantiene una potencia menor en comparación con Alfa y Theta. En conjunto, estos resultados indican que el protocolo de reposo se realizó de manera adecuada y que el voluntario logró mantenerse relajado, sin mostrar señales importantes de tensión o actividad cognitiva elevada.[5]
...
## Análisis del Estado de parpadeo y masticación constante
Este ejercicio fue planteado con el objetivo de registrar cambios significativos en las señales EEG a causa de artefactos frecuentes como las señales de movimientos faciales y oculares. Estos cambios pueden ser observados comparando la gráfica de amplitud versus tiempo de la Figura 4 con la de la Figura 3, donde se presentan aproximadamente 25 picos de similar amplitud en un tiempo de 10 segundos, por lo que la frecuencia principal del artefacto sería de 2,5 Hz. Esta frecuencia también está presente en la gráfica de la FFT de la Figura 4 con una magnitud de 40 dB, a diferencia de la FFT de la Figura 3, donde tiene una magnitud aproximada de 10 dB. Asimismo, la frecuencia aproximada de 2,5 Hz está dentro del rango de frecuencias del artefacto de movimiento ocular reportado en la literatura: 0-16 Hz [7].
Por otro lado, se ha reportado que el artefacto por movimiento facial suele sobreponerse a la señal de EEG en el rango de frecuencias de 20 a 30 Hz [8] en el lóbulo frontal, lo que se puede evidenciar en las gráficas de FFT: en la Figura 3, las frecuencias de ese rango tienen magnitudes de menos de 1 dB, mientras que en la Figura 4, tienen magnitudes de alrededor de 5 dB.
...
[7] “Decomposition and Reconstruction of Signal for Identification of Eye Movement Artifacts (OA) in Electroencephalogram (EEG) by applying Wavelet Transform,” IEEE Conference Publication | IEEE Xplore, Oct. 30, 2020. https://ieeexplore.ieee.org/document/9299616

[8] I.I Goncharova, D.J McFarland, T.M Vaughan, J.R Wolpaw, EMG contamination of EEG: spectral and topographical characteristics, Clinical Neurophysiology, Volume 114, Issue 9, 2003.
